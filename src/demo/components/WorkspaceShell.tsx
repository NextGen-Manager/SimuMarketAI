...
function Sidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[248px] flex-col border-r border-line bg-surface lg:flex">
      ...
      <div className="border-t border-line p-3"><div className="flex items-center gap-3 rounded-[10px] px-2 py-2"><span className="grid size-8 place-items-center rounded-full bg-teal-700 text-[11px] font-bold text-surface">{dashboardSeed.user.initials}</span><div className="min-w-0"><p className="truncate text-[12px] font-bold text-ink-900">{dashboardSeed.user.name} Pratama</p><p className="truncate text-[10.5px] text-ink-400">Akun demo pemilik</p></div></div></div>
    </aside>
  );
}
...
function MobileNav() {
  const pathname = usePathname();
  const items = [dashboardSeed.navigation[0].items[0], dashboardSeed.navigation[1].items[0], dashboardSeed.navigation[2].items[0], dashboardSeed.navigation[2].items[1]];
  return <nav aria-label="Navigasi seluler" className="fixed inset-x-0 bottom-0 z-50 grid h-[68px] grid-cols-4 border-t border-line bg-surface/95 px-2 backdrop-blur lg:hidden">{items.map((item) => { const active = pathname === item.href; return <Link key={item.label} href={item.href} aria-current={active ? "page" : undefined} className={`flex min-w-0 flex-col items-center justify-center gap-1 text-[10px] font-semibold ${active ? "text-teal-700" : "text-ink-400"}`}><NavIcon icon={item.icon} /><span className="truncate">{item.label === "Market Analysis" ? "Analisis" : item.label}</span></Link>; })}</nav>;
}

function Topbar() {
  return <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-line bg-surface/95 px-4 backdrop-blur sm:px-6 xl:px-8"><div className="flex items-center gap-2 lg:hidden"><BrandMark /><span className="hidden text-[14px] font-bold text-ink-900 sm:block">SimuMarket AI</span></div><label className="relative hidden max-w-[420px] flex-1 md:block"><span className="sr-only">Cari menu atau analisis</span><svg viewBox="0 0 24 24" className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-ink-400" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden><circle cx="11" cy="11" r="7" /><path d="m20 20-4-4" /></svg><input type="search" placeholder="Cari menu atau analisis..." className="h-9 w-full rounded-[9px] border border-line bg-surface-2 pl-9 pr-3 text-[12px] text-ink-700 placeholder:text-ink-400" /></label><div className="ml-auto flex items-center gap-2"><span className="rounded-full border border-amber-600/30 bg-amber-50 px-2.5 py-1 text-[10px] font-bold tracking-[0.08em] text-amber-600">MODE DEMO</span><span className="grid size-9 place-items-center rounded-full bg-teal-700 text-[11px] font-bold text-surface">{dashboardSeed.user.initials}</span></div></header>;
}
